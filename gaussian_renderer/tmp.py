...
def calculate_selection_score_delete(features, query_features, score_threshold=None, positive_ids=[0]):
        ...
        if scores.shape[-1] == 1:
            ...
            if score_threshold is None:
                q1 = torch.quantile(scores.to(torch.float), 0.25)
                q3 = torch.quantile(scores.to(torch.float), 0.75)
                score_threshold = q3 + 1.5 * (q3 - q1)
...
def render_edit(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, text_feature : torch.Tensor, edit_dict : dict,
    ...
    rendered_image, feature_map, radii, _ = rasterizer(
        means3D = means3D,
        means2D = means2D,
        shs = shs,
