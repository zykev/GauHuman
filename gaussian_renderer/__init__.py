...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, return_smpl_rot=False, transforms=None, translation=None):
    ...
    if not pc.motion_offset_flag:
        ...
        # opacity offset based on semantic feature
        opacity = pc.opacity_refiner(pc.get_opacity, pc.get_semantic_feature.squeeze(1))
    ...
    shs = None
    colors_precomp = pc.color_refiner(pc, viewpoint_camera, transforms.squeeze())
    # colors_precomp = None
